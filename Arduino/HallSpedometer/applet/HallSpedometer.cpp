...
float circum = 6.9375;           // Circumference of the tire in feet
int mph = 0;                // Variable to store the MPH
int revs = 0;                 // Used to keep the revolutions of the wheel
int revThreshold = 5;        // Decrease (increase) this number to decrease (increase) number of readings used in the average
...
// RPM = (60 seconds * Number of rotations since prior "lastSeen") / ((time since lastSeen) / 1000) in order go to seconds from milliseconds
  rpm = ((60.0 * revThreshold) / ((millis() - lastSeen)  / 1000.0));  // I don't know if that (float) is even necessary
  mph = (int) rpm * 60.0 * circum / 5280.0;  // RPM * 60 minutes/hour * circumference of the tire in feet / 5280 feet in a mile
